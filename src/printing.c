...
void        print_link(t_all *d, t_files *f)
{
    struct stat     *st;
    ...
    else
    {
        ...
        if (stat_file(f->link, st) == _SUCCESS)
        {
            ft_memdel((void *)&f->st);
            f->st = st;
            ft_printf(" -> ");
            print_color(f, f->st->st_mode, f->link);
            ft_printf("\n");
        }
        else
        {
...
void		long_format(t_all *d, t_files *f)
{
    ...
    (d->options & _S)?ft_printf("%*lu ",d->len[_BLOCK],f->st->st_blocks / 2): 0;
    ...
    print_time(d, f);
    (d->options & _G) ? print_color(f, f->st->st_mode, f->name) :
        ft_printf("%s", f->name);
    (f->link) ? print_link(d, f) : ft_printf("\n");
