...
int	_alpha_helper(char s1, char s2)
{
	s1 += (s1 >= 'A' && s1 <= 'Z') ? 32 : -32;
	s2 += (s2 >= 'A' && s2 <= 'Z') ? 32 : -32;
	return (s1 - s2);
...
int	ft_strhcmp(const char *s1, const char *s2)
{
	...
	while(!ret && (s1[i] || s2[i]))
	{
		ret = (ft_isalpha(s1[i]) && ft_isalpha(s2[i])) ?
			_alpha(s1[i], s2[i]) : _type(s1[i], s2[i]);
		i++;
	}
	if (ret)
		return (ret);
	...
	{
		ret = (ft_isalpha(s1[i]) && ft_isalpha(s2[i])) ?
			_alpha_helper(s1[i], s2[i]) : _type(s1[i], s2[i]);
		i++;
	}
	return (ret);
}
